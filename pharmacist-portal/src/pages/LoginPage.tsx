...
const LoginPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const success = await login(pNumber, password);
      if (success) {
        navigate('/manage-patient');
      } else {
        setError('Invalid P-number or password. Please try again.');
      }
    } catch (err) {
      ...
      <div className="login-container">
        <div className="login-card">
          <div className="login-header">
            <img 
              src="/mzansimed_logo.png" 
              alt="MzansiMed Logo" 
              className="login-logo"
            />
          </div>
          <p className="login-subtitle">Enter your credentials to access MzansiMed</p>
